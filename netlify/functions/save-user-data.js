// netlify/functions/save-user-data.js
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Consulta de ACTUALIZACIÓN. Ya no insertamos, solo actualizamos.
    await sql`
      UPDATE users
      SET 
        google_ads_id = ${adsId},
        connection_status = 'pending_approval', -- Actualizamos el estado
        updated_at = NOW()
      WHERE 
        netlify_id = ${user.sub};
    `;
